        ...
        rows: table.map((item) => (
            {
                ...
                Asignar: <Link to={{ pathname: "/Create/Payment", state: { _id: item._id, name: item.fName, newPayment: true, email: item.email, lName: item.lName, membership: item.Membresia } }} style={{ textDecoration: 'none' }}><button type="button" className="p-2 rounded BotonesColorCrear"><FontAwesomeIcon icon={faPlus} /></button></Link>
            }))
    };
    return (<div>
        <Header />
        <HeaderStatus
            h1="Busqueda del cliente"
            backUrl="/EmpleadoMenuPagos"
            backName="Menú de pagos"
            currentName="Busqueda del cliente"
        />
                    ...
                    <form onSubmit={findCustomer}>
                        ...
                        <div className="col-md-12 p-3 mt-5 ml-4" >
                            <div className="offset-md-2 col-md-7 ">
                                <h3 className="fw-bold text-center para-color ">Buscar cliente para crear un pago</h3>
                            </div>
